...
class FileViewer:
    def __init__(self, root):
        ...
        # Bind Command+Shift+Up for voice dictation of comments
        self.root.bind('<Command-Shift-Up>', self.voice_comment_dialog)
    ...
    def populate_favorites(self):
        ...
        # Ensure the folder open event is bound
    ...
    def voice_comment_dialog(self, event):
        """Open dialog for voice comment input"""
        ...
        comment_text = simpledialog.askstring(
            "Voice Comment",
            "Enter or paste your comment (use Fn+Fn for dictation):",
            parent=self.root
        )

        if comment_text and comment_text.strip():
            cell_content = self.cells[self.current_cell]
            self.add_comment(self.current_file, cell_content, self.current_cell, comment_text.strip())

            original_text = self.path_label.cget('text')
            self.path_label.config(text="✓ Comment saved")
    ...
    def read_next_comment(self, event):
        ...
        # Wrap around if we've read all comments
        if self.current_comment_reading_index >= len(comments):
            self.current_comment_reading_index = 0

        # Get the comment to read
        comment_text, created_at, confidence = comments[self.current_comment_reading_index]

        # Prepare text to read
        comment_number = self.current_comment_reading_index + 1
        ...
        try:
            ...
            # Show which comment is being read
